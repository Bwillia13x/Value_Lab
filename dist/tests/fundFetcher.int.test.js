"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const globals_1 = require("@jest/globals");
// fake env
process.env.SUPABASE_URL = 'https://dummy.supabase.test';
process.env.SUPABASE_ANON_KEY = 'dummy';
process.env.REDIS_URL = 'redis://localhost:6379';
// mock Supabase
globals_1.jest.mock('@supabase/supabase-js', () => ({
    createClient: globals_1.jest.fn(() => ({
        from: () => ({
            select: () => ({
                eq: () => ({
                    gte: () => ({
                        order: () => ({
                            limit: () => Promise.resolve({ data: [] }),
                        }),
                    }),
                }),
            }),
            insert: () => Promise.resolve({}),
        }),
    })),
}));
// in-memory Redis
globals_1.jest.mock('ioredis', () => globals_1.jest.fn().mockImplementation(() => {
    const store = new Map();
    return {
        get: async (k) => {
            const e = store.get(k);
            if (!e)
                return null;
            if (e.exp && e.exp < Date.now()) {
                store.delete(k);
                return null;
            }
            return e.v;
        },
        set: async (k, v, mode, ttl) => {
            store.set(k, {
                v,
                exp: mode === 'EX' ? Date.now() + ttl * 1000 : undefined,
            });
            return 'OK';
        },
    };
}));
const fundFetcher_1 = require("../src/services/fundFetcher");
globals_1.jest.setTimeout(30000);
describe('fundFetcher integration', () => {
    const tickers = ['BRK-A', 'SPY'];
    it('fetches & caches in <2 s', async () => {
        for (const t of tickers) {
            await (0, fundFetcher_1.getFundReturns)(t); // warm
            const t0 = Date.now();
            const s = await (0, fundFetcher_1.getFundReturns)(t); // cached
            expect(s.length).toBeGreaterThan(5);
            expect(Date.now() - t0).toBeLessThan(2000);
        }
    });
});
